...
const data = await queryContent('retrospectives').sort({ date: -1 }).find()
    ...
    <div class="grid grid-cols-1 justify-center gap-4 sm:grid-cols-[repeat(auto-fit,75%)] md:grid-cols-[repeat(auto-fit,48%)] lg:gap-6">
      ...
      >
        <div class="relative aspect-[240/135] w-full rounded-t-xl bg-neutral-100 dark:bg-neutral-900">
          <img
            :alt="item.navigation.title" loading="lazy" decoding="async" data-nimg="fill" class="rounded-t-xl bg-[url('/images/fallback.webp')] bg-cover bg-center bg-no-repeat object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw"
            :src="item.navigation.image" style="position: absolute; height: 100%; width: 100%; inset: 0px; color: transparent;"
          >
        </div>
        <span class="relative z-10 flex w-full flex-1 shrink-0 flex-col justify-between gap-0.5 rounded-b-xl px-3 py-2 md:gap-0">
          <h2 class="z-20 text-base font-bold transition-opacity md:text-xl">{{ item.navigation.title }}</h2>
          <span class="relative z-20 flex items-center justify-between opacity-80 transition-opacity">
            <span class="inline-flex items-center space-x-3">
              <span class="inline-flex items-center space-x-1 text-[12px] font-medium md:text-sm">
                <span>{{ diffTime(item?.navigation?.date) }} 前  #{{ $dayjs(item?.navigation?.date).format('MM') }}</span>
              </span>
            </span>
          </span>
        </span>
